...
import { getAgentIcon, copyToClipboard, formatTime } from '../utils/helpers';
...
export const Message: React.FC<MessageProps> = ({ 
  ...
  const { getUserMessageVersionIndex, setUserMessageVersionIndex } = useAppStore();
  ...
  // 使用ref来跟踪是否已经修正过索引，避免重复修正
  // 使用消息ID和消息数量作为key，确保在消息变化时重新计算
  const indexCorrectedRef = useRef<string | null>(null);
  
  const isUser = message.role === 'user';
  const citations = message.citations || message.sources || [];
  
  // 获取当前版本索引（从store中读取）
  const storedVersionIndex = getUserMessageVersionIndex(message.id);
  
  // 对于助手消息，需要获取父节点的版本索引
  // 先找到父消息，然后获取其版本索引
  const parentMsg = React.useMemo(() => {
    if (!isUser && message.parent_id) {
      const parentId = typeof message.parent_id === 'number' 
        ? message.parent_id 
        : parseInt(String(message.parent_id));
      return messages.find(m => {
        const msgId = m.message_id || parseInt(m.id);
  ...
  const versionInfo = useMemo(() => {
    ...
    let shouldShowAssistant = true;
    
    if (isUser && !message.parent_id) {
      // 用户消息（根节点）：查找所有子节点（助手回复）作为版本
      const userMsgId = message.message_id || parseInt(message.id);
      const children = messages.filter(m => {
        if (m.role !== 'assistant' || !m.parent_id) return false;
        // 比较parent_id（可能是数字或字符串）
  ...
  useEffect(() => {
    ...
    // 如果已经修正过，且messages长度没变化，就不需要再次修正
    const messageKey = `${message.id}_${messages.length}`;
    if (indexCorrectedRef.current === messageKey) {
      return;
    }
  ...
  }, [isUser, message.parent_id, message.message_id, message.id, messages.length]); // 移除storedVersionIndex依赖，避免循环
  
  const handleVersionPrev = () => {
    if (currentVersionIndex > 0) {
      setUserMessageVersionIndex(message.id, currentVersionIndex - 1);
    }
  };
  ...
  // 对于助手消息，如果不应该显示，返回null
  if (!isUser && !shouldShowAssistant) {
    return null;
  }
    ...
    <div className={`message message-${message.role}`} data-message-id={message.id}>
      ...
      {!isUser && message.agent_type && (
        <div className="agent-badge">
          <span>{getAgentIcon(message.agent_type)}</span>
          <span style={{ textTransform: 'capitalize' }}>
            {message.agent_type.replace('_', ' ')}
          </span>
        </div>
      )}
      
      <div className={isUser ? 'message-user-editable' : ''}>
        {/* Message Content */}
        {isEditing ? (
          <div className="message-edit-form">
            ...
            <div className="message-edit-actions">
              <button className="edit-action-btn edit-cancel-btn" onClick={handleCancelEdit}>
                取消
              </button>
              <button className="edit-action-btn edit-save-btn" onClick={handleSaveEdit}>
                保存并重新生成
              </button>
            </div>
          </div>
        ...
        {isUser && !message.parent_id && !isEditing && (
          <div className="user-message-actions">
            ...
            <button className="user-action-btn copy-btn" title="复制" onClick={handleCopy}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      {!isUser && shouldShowAssistant && (
        <div className="message-actions">
          ...
          <div className="message-more-menu-wrapper">
            <button 
              className="message-action-icon-btn"
            ...
            {showMoreMenu && (
              <div className="message-more-menu" style={{ display: 'block' }}>
                <button 
                  className="more-menu-item"
                  onClick={() => {
                    onBranch && onBranch(message.content);
          ...
          {citations.length > 0 && (
            <div className="message-citations">
              ...
              <div className="citations-icons">
                {citations.map((citation, idx) => (
                  <img
                    key={idx}
                    src={`https://www.google.com/s2/favicons?domain=${encodeURIComponent(citation.url || '')}&sz=32`}
                    alt={citation.title || 'Source'}
